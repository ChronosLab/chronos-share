...
const Page: React.FC = () => {
    ...
    const mint = async (nftName: string, nftSymbol: string, nftAmount: string, nftPrice: string) => {
        try {
            ...
            const result = await mintNFT(nftName, "https://raw.seadn.io/files/2e35f09a9ed211e3f45f469e78c30a46.png", nftSymbol, nftAmount, nftPrice);
        ...
        <div className="flex flex-col min-h-screen">
            ...
            {mintSuccess && (
                <div className="w-full flex justify-center items-center">
                    <div role="alert" className="flex justify-center items-center alert alert-success mt-6 mx-6 w-1/2">
                        ...
                        <span>Your NFT has been minted! Transaction: <a href={`https://sepolia.etherscan.io/tx/${txHash}`}
                                                                        target="_blank" rel="noopener noreferrer"
                                                                        className="underline text-blue-600 hover:text-blue-800 visited:text-purple-600">{txHash.substring(0, 20)}...</a></span>
            ...
            <div className="container mx-auto w-1/2">
                {account ? (
                    ...
                    <button className="btn btn-info my-4" onClick={connectWallet}>Connect to MetaMask</button>
                )}
                <div className="form-control">
                    ...
                    <input type="text" value={nftName} onChange={(e) => setNftName(e.target.value)}
                           placeholder="NFT Name" className="input input-bordered"/>
                    <label className="label">
                        <span className="label-text">Enter NFT Symbol(10):</span>
                    </label>
                    <input type="text" value={nftSymbol} onChange={(e) => setNftSymbol(e.target.value)}
                           placeholder="NFT Symbol" className="input input-bordered"/>
                    <label className="label">
                        <span className="label-text">Enter NFT Amount:</span>
                    </label>
                    <input type="text" value={nftAmount} onChange={(e) => setNftAmount(e.target.value)}
                           placeholder="1" className="input input-bordered"/>
                    <label className="label">
                ...
                <div className="form-control">
                    ...
                    <input type="text" value={buyTokenSymbol} onChange={(e) => setBuyTokenSymbol(e.target.value)}
                           placeholder="Token Name" className="input input-bordered"/>
                    <label className="label">
                        <span className="label-text">Enter Buy Token Amount:</span>
                    </label>
                    <input type="text" value={buyTokenAmount} onChange={(e) => setBuyTokenAmount(e.target.value)}
                           placeholder="1" className="input input-bordered"/>
                </div>

                <button className="btn btn-secondary my-4" onClick={() => buy(buyTokenSymbol, account as `0x${string}`, buyTokenAmount)}>
                    Buy NFT
                </button>
            </div>
        </div>
